...
class ControllerExtensionPaymentTwoCheckout extends Controller {
	...
	public function callback() {
		...
		if (strtoupper(md5($this->config->get('payment_twocheckout_secret') . $this->config->get('payment_twocheckout_account') . $order_number . $this->request->request['total'])) == $this->request->request['key']) {
			if ($this->currency->format($order_info['total'], $order_info['currency_code'], $order_info['currency_value'], false) == $this->request->request['total']) {
				$this->model_checkout_order->addOrderHistory($this->request->request['cart_order_id'], $this->config->get('payment_twocheckout_order_status_id'));
			} else {
